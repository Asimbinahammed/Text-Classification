...
def feature_vecs_NLP(train_pos, train_neg, test_pos, test_neg):
    ...
    # Removing words that occur multiple times in each sentence and considering it only once
    
    train_pos_unique = [list(set(i)) for i in train_pos]
    train_neg_unique = [list(set(i)) for i in train_neg]
    test_pos_unique = [list(set(i)) for i in test_pos]
    test_neg_unique = [list(set(i)) for i in test_neg]
    
    # Removing stop words from all the lists
    train_pos_stp = [[w for w in data if w not in stopwords] for data in train_pos_unique]
    train_neg_stp = [[w for w in data if w not in stopwords] for data in train_neg_unique]
    test_pos_stp = [[w for w in data if w not in stopwords] for data in test_pos_unique]
    test_neg_stp = [[w for w in data if w not in stopwords] for data in test_neg_unique]
    
    # Flattened train_pos and train_neg lists
    train_pos_flat = [item for sublist in train_pos_stp for item in sublist]
    train_neg_flat = [item for sublist in train_neg_stp for item in sublist]
    
    train_pos_count = collections.Counter(train_pos_flat)
    train_neg_count = collections.Counter(train_neg_flat)
    
    
    # Removing words that are not present in 1% of either positive or negative texts
    pos_1_count = len(train_pos_stp)/100
    neg_1_count = len(test_pos_stp)/100
    
    pos_filtered_words = {k:v for k,v in train_pos_count.iteritems() if v >= pos_1_count}
    neg_filtered_words = {k:v for k,v in train_neg_count.iteritems() if v >= neg_1_count}
    
    # Removing words that are not present at least twice as many positive texts as negative texts
    pos_filtered_words_2 = {k:v for k,v in pos_filtered_words.iteritems() if v >= 2*neg_filtered_words.get(k,0)}
    neg_filtered_words_2 = {k:v for k,v in neg_filtered_words.iteritems() if v >= 2*pos_filtered_words.get(k,0)}
    
    # Extracting the unique words from both these dictionaries
    
    feature_word_list = list(set(pos_filtered_words_2.keys()).union(set(neg_filtered_words_2.keys())))
    ...
    for i, sentence in enumerate(train_pos):
        train_pos_vec.append([1 if feature in sentence else 0 for feature in feature_word_list])
    
    for i, sentence in enumerate(train_neg):
        train_neg_vec.append([1 if feature in sentence else 0 for feature in feature_word_list])
    
    for i, sentence in enumerate(test_pos):
        test_pos_vec.append([1 if feature in sentence else 0 for feature in feature_word_list])
    
    for i, sentence in enumerate(test_neg):
        test_neg_vec.append([1 if feature in sentence else 0 for feature in feature_word_list])
...
def feature_vecs_DOC(train_pos, train_neg, test_pos, test_neg):
    ...
    # YOUR CODE HERE
    train_pos_vec = [model.docvecs['TRAIN_POS_'+str(i)] for i in range(len(train_pos))]
    train_neg_vec = [model.docvecs['TRAIN_NEG_'+str(i)] for i in range(len(train_neg))]
    test_pos_vec = [model.docvecs['TEST_POS_'+str(i)] for i in range(len(test_pos))]
    test_neg_vec = [model.docvecs['TEST_NEG_'+str(i)] for i in range(len(test_neg))]
...
def build_models_NLP(train_pos_vec, train_neg_vec):
    ...
    # Fitting BernoulliNB to training data
    nb_model = sklearn.naive_bayes.BernoulliNB(alpha = 1,binarize = None)
    nb_model.fit(X,Y)
...
def build_models_DOC(train_pos_vec, train_neg_vec):
    ...
    # Fitting a LogisticRegression to training data
    lr_model = sklearn.linear_model.LogisticRegression()
    lr_model.fit(X,Y)
...
def evaluate_model(model, test_pos_vec, test_neg_vec, print_confusion=False):
    ...
    # YOUR CODE HERE
    # Test_Pos_prediction
    Test_Pos_prediction = model.predict(test_pos_vec)
    Counter_pos = collections.Counter(Test_Pos_prediction)
    tp, fn = Counter_pos['pos'], Counter_pos['neg']
    
    # Test_Neg_prediction = model.predict(test_neg_vec)
    Test_Neg_prediction = model.predict(test_neg_vec)
    Counter_neg = collections.Counter(Test_Neg_prediction)
    tn, fp = Counter_neg['neg'], Counter_neg['pos']
